...
这样的操作被称为**order-execute架构**；它要求所有节点都执行每一笔交易，并且所有的交易都必须是确定性的（不能产生不同的结果）。市面上几乎所有的区块链项目都是基于这样的架构，这导致了几个问题：
